...
class MultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, x, pos_kk, mask):
        ...
        if self.is_initialized:  # 初始化
            if self.is_first_frame:  # 第一帧
                device = q.device
                k_init = torch.zeros(q.size(0), q.size(1), max_cache_len, q.size(3)).to(device)
                v_init = torch.zeros(q.size(0), q.size(1), max_cache_len, q.size(3)).to(device)
                self.past_key_value = (k_init, v_init)
